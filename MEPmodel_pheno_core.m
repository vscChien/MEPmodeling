function sim = MEPmodel_pheno_core(ref,p)
    ...
    for i=1:nIntensities
        ...
        tshift = p(3);
        N = round(p(3+i)); 
        spike_times{i} = gen_spike_times_gamma(shape,rate,N) + (nIntensities-i)*tshift;   
    ...
    sim.axonalDelday=axonalDelay+t0(1);
    sim.shape = shape; 
    sim.rate = rate; 
    sim.tshift = tshift;
end
...
function spike_times = gen_spike_times_gamma(shape,rate,N)

     scale = 1/rate;
     spike_times = gaminv(linspace(0, 0.99, N), shape, scale);
