...
j = 1;
 while j <= loop
     %fprintf('step %d :\n',j);
     J = NMM_diff_A_lfm(Para_E_test,r_test,func,y_goal); % original
     %J = NMM_diff_A_lfm_pade(Para_E_test,r_test,func,y_goal); % Pade 1st order (fast)
     %J = NMM_diff_A_lfm_pade33(Para_E_test,r_test,func,y_goal); % Pade 3rd order (slow)
